...
function counter() {
  ...
  var count = 1;
return function(){
  return count++;//si incremnto el valor dewspues del returm no s eva realizar pórqu el return cierra la ejecuccion.
}

}
var oContador=counter();
oContador();
...
function cacheFunction(cb) {
  ...
  Tu tarea aquí es lograr, mediante un closure, que cacheFunction actúe como una memoria caché
   para el callback que recibe por parámetro (cb); es decir, que "recuerde" 
   el resultado de cada operación que hace, de manera que, al realizar 
   una operación por segunda vez, se pueda obtener el resultado de esa "memoria" 
 ...
 return function(argumento){
  if(varcache.hasOwnProperty(argumento)){
    return varcache[argumento]
  }
  else {
    varcache[argumento]=cb(argumento);
    return varcache[argumento];
  }
 }
}
